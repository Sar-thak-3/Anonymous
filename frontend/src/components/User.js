...
const User = () => {
  ...
  return (
    <div>
      {createpost && <Createpost cancel={setCreatepost}></Createpost>}
      <div className="except-create" style={styles}>
      ...
      {ress && ress.map((post)=>{
        return <Allposts key={post._id} pos={"left"} allcontent={post}></Allposts>
      })}
      </div>
    </div>
  );
